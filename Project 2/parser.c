...
static void gettoken(void)
{
    *chararray = '\0';
    ...
    if (nextch == '<') // input redirection
    {
        token = T_FROMFILE;
        return;
    }
    else
    if (nextch == '>') // output redirection
    {
        get();
        if (nextch == '>')
        {
    ...
    if (nextch == ';') // sequential
    {
        token = T_SCOLON;
        return;
    }
    else
    if (nextch == '&') // and-conditional
    {
        get();
        if (nextch == '&')
        {
...
static bool get_redirection(SHELLCMD *t1)
{
    ...
    if (token == T_SQUOTE || token == T_DQUOTE)
    {
...
static SHELLCMD *
cmd_pipeline(void); // a forward declaration
...
#endif /* if defined(CGI_INTERFACE) */
